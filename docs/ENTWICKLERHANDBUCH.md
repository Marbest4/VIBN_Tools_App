...
Die FEE-Runtime ist lazy. `Services.Initialize()` darf weder `CoreApi` erzeugen noch Interfaces laden. Nur der explizite Connect-Ablauf darf `Services.TryInitializeFeeApi(...)` aufrufen; Interface-/Signalabfragen benötigen anschließend `Connection.CanUseFeeFeatures` und einen ausdrücklichen Benutzerbefehl. Damit bleiben Start und reine Offline-Funktionen unabhängig von installierter beziehungsweise erreichbarer FEE-Infrastruktur.
