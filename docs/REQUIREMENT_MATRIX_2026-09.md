...
| Grob Generation Interface verwenden | vorhanden, FEE-Live-Abnahme offen | Resolver/Executor/FEE wrappers, unverändertes Pluginprojekt | Name, Provider-GUID und Provider gemeinsam prüfen; vorhandene Signale wiederverwenden, fehlende dort erstellen; bei Widerspruch vor BasicFrame abbrechen | Plugin-/Solution-Build 0/0 und Policytests; FEE-Live-Abnahme offen |
...
| AI-Reiter Regelvorschläge | vorhanden | AI View/VM, `RuleSuggestionService`, ReviewStore | eigener Subtab mit Status und Accept/Reject; kein automatischer Requirements-Schreibzugriff | Aggregations-, Persistenz- und WPF-Bindingtest |
...
| FEE2Container | provenance- und variablenbasierte Version implementiert; Slot-Rückmapping und Live-Abnahme offen | `FeeContainerProvenanceCodec`, `FeeContainerVariableProjector`, `Fee2ContainerService`, Page VM/View, Visual Executor | BasicFrame-Root-Auswahl, persistente versionierte SDK-Tags, Entry→Variable-GUID, aktuelle Signal-/Adress-/Pfad-/Typ-/ID-Projektion, Prüfsumme, Vorschau und atomarer XML-Export; ältere/manuelle Modelle bewusst ausgeschlossen; nachträgliche Slot-Umbauten noch nicht zurückprojiziert | automatischer Auswahl-/Prüfsummen-/Variablenänderungs-/PLC_IN-Round-Trip und WPF-Smoke; Move-/Slot-Ketten sowie Persistenz nach echtem FEE-Save/Reload offen |
