...
| PLC_IN definierte Mehrfachbelegung | Fachregel geklärt, Implementierung offen | SlotPolicy, `ContainerBaseClass`, FEE-Mapping, `FeeSimpleMove` | jede Mehrfachbelegung zulassen, aber je Signal ein eigenes Move erzeugen und verlustfrei auf den gemeinsamen Eingang abbilden; Validierung und Executor atomar ändern | Sollfixture mit mehreren Signalen, eindeutigen Moves und vollständigen Slotkanten |
...
| Grob Generation Interface verwenden | Identität geklärt; Build extern blockiert | Discovery/Executor/FEE wrappers, unverändertes Pluginprojekt | Name, GUID und Provider gemeinsam prüfen; passende Signale wiederverwenden, fehlende dort erstellen; bei Widerspruch vorab abbrechen | Fake-/Integrationstest; Plugin-Build nach Bereitstellung von `FS.SDK.Localization.dll`; FEE-Live-Abnahme |
